...
public:
			 ...
			 size_t low_order)
  {
	...
	for (size_t j = 0; j <= high_order; j ++) {
	  ...
	  // for (size_t k = 0; k <= high_order; k ++) {
	    // double l = mesh.quadrature[high_order]->cardinal[k].value(value(xi));

	    // real depth = offset + ((xi + 1.0)/2.0 * thickness);
	      
	    // cell1.nodes[j].rho += projected_nodes[k].rho(depth) * l;
	    // cell1.nodes[j].A += projected_nodes[k].A(depth) * l;
	    // cell1.nodes[j].C += projected_nodes[k].C(depth) * l;
	    // cell1.nodes[j].F += projected_nodes[k].F(depth) * l;
	    // cell1.nodes[j].L += projected_nodes[k].L(depth) * l;
	    // cell1.nodes[j].N += projected_nodes[k].N(depth) * l;
	  // }
