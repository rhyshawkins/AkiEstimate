...
int main(int argc, char *argv[])
{
  ...
  for (int i = (fsamples - 1); i >= 0; i --) {
	...
	phasestd[i] += (model_errors[j]*model_errors[j]) * (dcdp*dcdp);
      ...
      //
      group[i] = (2.0*normB*k + normC)/(2.0*omega*normA);
      if (k < 0.0) {
	group[i] = -group[i];
      }
      for (int j = 0; j < nparameters; j ++) {
	groupstd[i] += (model_errors[j]*model_errors[j]) * (dUdp(j, 0)*dUdp(j, 0));
      ...
      double disc2 = k*k - omega*omega/vp2;
      
      if (isnormal(disc1) && disc1 > 0.0 &&
	  isnormal(disc2) && disc2 > 0.0) {
	
	double newscale2 = sqrt(disc2);
	
	//
	// Experiments have shown that using the vp scale for both laguerre elements
	// is the most accurate
	//
	rayleigh.recompute(mesh, boundaryorder, newscale2, newscale2);
      }	
